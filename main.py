...
class Projector:
    def __init__(self, step, sample_num):
        self.num_steps                  = step
        self.dlatent_avg_samples        = sample_num
...
def main():
    st.title("""StyleGAN2 + ADA + FreezeD""")
    st.title("""KangpoolFace Translator""")
    st.subheader('실제 인물사진을 입력하면 강풀작가 화풍으로 출력합니다.')
    st.subheader('')
    st.subheader('Steps수 에 비례하여 출력시 소요되는 시간이 늘어납니다.')
    st.subheader('250 Steps 경우 약 1분정도의 시간이 소요됩니다.')

    st.sidebar.header('Argument')

    def user_input_features():
        target_fname = st.sidebar.file_uploader('입력 이미지', type=['png','jpg','jpeg'])
        samples = st.sidebar.slider('평균 학습 이미지',10000,100000,10000)
        steps = st.sidebar.slider('스텝 수',100,2000,250)
        data = {'target_fname' : target_fname,
                'samples' : samples,
                'steps' : steps}
        return data
    ...
    # Load target image.
    target_pil = PIL.Image.open(data['target_fname'])
    w, h = target_pil.size
    s = min(w, h)
    ...
    proj = Projector(step = data['steps'], sample_num = data['samples'])
    proj.set_network(Gs)
    ...
    result = PIL.Image.open(f'{outdir}/proj.png')
